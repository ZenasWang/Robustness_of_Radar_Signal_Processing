function [ targetList, pulse_compression] = signalProcessing( rawData, radarParameters )
...
for actRangeTarg = 1 : numel(rangeSpecMaxPos)
    ...
    actVelSpec = fftshift(fft(fft_range(actRangeBin,:,:), size(radarData, 2),2)); % sqrt(size(radarData, 2))
