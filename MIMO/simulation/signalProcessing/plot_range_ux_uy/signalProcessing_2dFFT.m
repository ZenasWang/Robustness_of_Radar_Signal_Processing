...
numTrainingCells_range = 6;
numTrainingCells_doppler = 6;

numGuardCells_range = 2;
numGuardCells_doppler = 2;
...
% 2D FFT
arrayResponse_3D = fft2(radarData,size(radarData, 1), ...
                        size(radarData, 2));
...
cfar = phased.CFARDetector2D('Method', 'OS', 'GuardBandSize',[numGuardCells_range numGuardCells_doppler],...
    'TrainingBandSize',[numTrainingCells_range numTrainingCells_doppler],...
    'ProbabilityFalseAlarm', probabilityFalseAlarm, ...
      'Rank', 15, 'OutputFormat','Detection index');
cutidx = [];
range_start = numTrainingCells_range + numGuardCells_range + 1;
range_end = size(arrayResponse_3D, 1) - (numTrainingCells_range + numGuardCells_range);
doppler_start = numTrainingCells_doppler + numGuardCells_doppler + 1;
doppler_end = size(arrayResponse_3D, 2) - ( numTrainingCells_doppler + numGuardCells_doppler);

for i = range_start:range_end
    for j = doppler_start:doppler_end
        cutidx = [cutidx,[i;j]];
    end
end
ncutcells = size(cutidx,2);

det_index = cfar(sum(abs(arrayResponse_3D).^2, 3), cutidx);
clusterIDs = helperAutoDrivingRadarSigProc('Cluster Detections', det_index);

range_doppler_fine = fft2(radarData, zeroPadingFactor*size(radarData, 1), ...
                    zeroPadingFactor*size(radarData, 2));

range_doppler_fine = squeeze(sum(abs(fftshift(range_doppler_fine, 2)).^2, 3));
for i = 1: max(clusterIDs)
    idx = find(clusterIDs == i);
    target = det_index(:, idx);
    range_idx = target(1,1);
    doppler_idx = target(2,1);
    spectral = range_doppler(target(1,1), target(1, 2)); 
    for j = 1 : size(target, 2)
        if range_doppler(target(1,i), target(2, i)) > spectral
            spectral = range_doppler(target(1,i), target(2, i));
            range_idx = target(1,i);
            doppler_idx = target(2,i);
        end
    end
    range_doppler_fine(range_idx, doppler_idx);
    
    % interpretation for 2D DOA
    range_fine = range_idx*3-3 : 1 : ux(ux_ind+3);
